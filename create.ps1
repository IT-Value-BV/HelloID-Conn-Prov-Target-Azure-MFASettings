...
$outputContext.AccountReference = New-Guid
...
try {
    ...
    if ($actionContext.CorrelationConfiguration.Enabled) {
        ...
        Try {
            $Uri = "https://graph.microsoft.com/v1.0/users/$($correlationValue)"
            $correlatedAccount = Invoke-RestMethod @AADMethod -Method 'Get' -Uri $Uri
        }
        catch {
            throw "Couldn't get Azure User with UPN: $($correlationValue) ($($_))"
        }
        ...
        throw "Correlation needs to be configured for this system"
    }
    ...
    $BaseUri = "https://graph.microsoft.com/beta/users/$($outputContext.AccountReference)/authentication"
